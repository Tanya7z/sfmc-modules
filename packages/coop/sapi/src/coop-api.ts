/**
 * @sfmc-bds/module-coop — coop-api.ts
 *
...
export interface CoopRow extends Record<string, unknown> {
  cid: string;
  name: string;
...
export interface CoopMemberRow extends Record<string, unknown> {
  cid: string;
  player_id: string;
...
export interface CoopAccountRow extends Record<string, unknown> {
  cid: string;
  balance: number;
  version: number;
  updated_at: number;
...
export interface CoopShopItemRow extends Record<string, unknown> {
  id: string;
  cid: string;
  name: string;
  item_type: string;
...
export interface CoopShopGroupRow extends Record<string, unknown> {
  groupid: string;
  displayname: string;
  type_function: string;
}

export interface CoopBankLogRow extends Record<string, unknown> {
  id: string;
  cid: string;
  actor_id: string;
  actor_name: string;
...
/** 域模型：成员（API 视图，可与 CoopMemberRow 并存） */
...
export async function createCoop(
  ...
  actorId: string,
  actorName: string
): Promise<{ ok: boolean; coop?: CoopRow | undefined; error?: string | undefined }> {
  const now = Date.now();
  try {
