/**
 * @sfmc-bds/module-coop — coop-core.ts
 *
...
export async function buy(gid: string, num: number, player: Player): Promise<{ ok: boolean; error?: string | undefined }> {
...
export async function sell(gid: string, num: number, player: Player): Promise<{ ok: boolean; error?: string | undefined }> {
  const all = await _getAllShopItems();
  const good = all.find((e) => e.id === gid);
