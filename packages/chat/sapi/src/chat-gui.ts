...
import type { Channel, RedPacket } from "./chat-api.js";
import { DogeChat } from "./doge-chat.js";
import { Money } from "@sfmc-bds/sdk/sapi/runtime";
import { FormStatus, MenuNavigator, Page, obsNum, obsStr } from "@sfmc-bds/sdk/sapi/runtime";
...
export class ChatGUI {
  ...
  private async buildPanel(page: Page): Promise<void> {
  ...
  private buildRename(page: Page): void {
    const status = new FormStatus(page);
    const channel = this.nav.state.channel as Channel;
    if (!channel) {
      page.label("频道数据丢失。");
  ...
  private async buildPickPlayer(page: Page): Promise<void> {
    const status = new FormStatus(page);
  ...
  private async buildRedPacket(page: Page): Promise<void> {
  ...
  private buildSendRedPacket(page: Page): void {
    const status = new FormStatus(page);
    const amount = obsStr("");
  ...
  private async buildClaimRedPacket(page: Page): Promise<void> {
    const status = new FormStatus(page);
  ...
  private buildQuickSendPrivate(page: Page): void {
    const status = new FormStatus(page);
    const amount = obsStr("");
  ...
  private buildQuickSendGroup(page: Page): void {
    const status = new FormStatus(page);
    const amount = obsStr("");
    const count = obsStr("1");
    page.textField("金额", amount, { description: "输入红包总金额" });
