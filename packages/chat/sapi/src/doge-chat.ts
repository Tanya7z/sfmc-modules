...
/** 频道聊天原文下发；需保留自定义颜色/前缀，不能走 Msg.* */
function sendChatRaw(player: Player, message: ChatRawPayload): void {
  // eslint-disable-next-line @sfmc-bds/no-player-send-message -- 频道消息需保留原始格式，不能加 Msg 前缀
...
export class DogeChat {
  ...
  static async loadChannelHistory(player: Player, channelId: string): Promise<void> {
    ...
    if (history.length === 0) {
      sendChatRaw(player, `§7--- §f${channel.prefix} §7频道暂无历史消息 ---`);
      return;
    }
    sendChatRaw(player, `§7--- §f${channel.prefix} §7频道历史消息 ---`);
